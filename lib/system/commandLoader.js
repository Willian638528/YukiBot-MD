...
async function seeCommands(dir = commandsFolder) {
  ...
  for (const fileOrFolder of items) {
    ...
    try {
      ...
      console.error(chalk.red(`❌ Erro de sintaxe em \( {fileOrFolder}:\n \){err.message}`))
      continue
    }
    ...
    try {
      ...
      comando.command.forEach(cmd => {
        global.comandos.set(cmd.toLowerCase(), {
          pluginName,
          run: comando.run,
          category: comando.category || "sem categoria",
      ...
      console.error(chalk.red(`❌ Erro no plugin ${fileOrFolder}:`), e)
    }
  }
}
...
globalThis.reload = async (_ev, filename) => {
  ...
  if (!fs.existsSync(fullPath)) {
    console.log(chalk.yellow(`⚠ Plugin removido: ${filename}`))
  ...
  try {
    ...
    console.error(chalk.red(`❌ Erro ao recarregar ${filename}:\n`), e)
  }
}
