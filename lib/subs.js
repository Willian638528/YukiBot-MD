  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, isNewLogin, qr }) => {
    ...
    if (connection === 'open') {
      ...
      /*if (m && client && isCommand && commandFlags[senderId]) {
        client.sendMessage(m.chat, { text: `✎ Você conectou um novo Socket do tipo *Gratuito*.` }, { quoted: m })
    ...
    if (connection === 'close') {
      ...
      if ([401, 403].includes(reason)) {
        if (intentos < 5) {
          console.log(
            chalk.gray(
              `[ ✿  ]  SUB-BOT ${botId} Conexão fechada (código ${reason}) tentativa ${intentos}/5 → Tentando novamente...`,
            ),
          )
          setTimeout(() => {
          ...
          console.log(
            chalk.gray(`[ ✿  ]  SUB-BOT ${botId} Falhou após 5 tentativas. Excluindo sessão.`),
          )
          try {
            fs.rmSync(sessionFolder, { recursive: true, force: true })
          } catch (e) {
            console.error(`[ ✿  ] Não foi possível excluir a pasta ${sessionFolder}:`, e)
          }
    ...
    if (qr && isCode && phone && client && chatId && commandFlags[senderId]) {
      try {
        ...
        console.error("[Erro no Código]", err);
      }
    }
